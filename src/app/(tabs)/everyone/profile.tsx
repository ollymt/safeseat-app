...
export default function Profile() {
	...
	useEffect(() => {
		const unsubscribe = navigation.addListener("beforeRemove", (e) => {
			...
			// stop the navigation from happening immediately
			e.preventDefault();
			...
			Alert.alert(
				...
				[
					{ text: "Stay", style: "cancel" },
					{
						text: "Discard",
						style: "destructive",
						// re-fire the exact navigation action that was blocked
	...
	useEffect(() => {
		if (editMode && !skipNextConversion.current) {
			...
			return; // don't run on the render where we just entered edit mode
		}
		if (!editMode) {
		...
		if (!isMetric) {
			...
			// --- 2. Weight Sync (Imperial -> Metric) ---
			const lbsNum = parseFloat(tempLbs);

			if (!isNaN(lbsNum) && lbsNum >= 0) {
				const computedKg = lbsNum / 2.20462;
				setWeight(String(Math.round(computedKg * 10) / 10)); // Maintain 1 decimal place accuracy
			} else if (tempLbs === "") {
				setWeight("Not Set");
			}
		}
	...
	const hasUnsavedChanges = () => {
		...
		return (
			userName !== orig.name ||
			userIcon !== orig.icon || // <-- ADD THIS LINE
			height !== orig.height ||
			weight !== orig.weight ||
	...
	const handleSaveChanges = async () => {
		...
		try {
			...
			const updatedProfile = {
				name: userName,
				icon: userIcon, // Already present in your local cache object
				email: userEmail,
				phone: userPhone,
			...
			const firestorePayload: any = {
				name: userName,
				icon: userIcon, // <-- ADD THIS LINE to save the Base64 image to Firestore
				email: userEmail,
				phone: userPhone,
				bloodType: bloodType,
				allergies: allergies,
	...
	const handlePickImage = async () => {
		...
		const result = await ImagePicker.launchImageLibraryAsync({
			...
			allowsEditing: true,
			aspect: [1, 1],
			quality: 0.3, // Compress heavily to keep Base64 string well under 1MB
			base64: true, // Requests Base64 output from Expo
		});

		if (!result.canceled && result.assets[0].base64) {
			// Construct standard Data URI string
			const base64Image = `data:image/jpeg;base64,${result.assets[0].base64}`;
			setUserIcon(base64Image);
		}
	};

	// Wipes stale data immediately when switching profiles so old profile data never renders
	useEffect(() => {
		setUserName("");
		setUserIcon("");
		setUserEmail("");
	...
	useFocusEffect(
		useCallback(() => {
			...
			// Temporarily flag as loading while async SecureStore reads
			setIsLoaded(false);
								...
								<Image
									source={
										!userIcon || userIcon === "Not Set" || userIcon === ""
											...
											: { uri: userIcon } // React Native Image natively supports base64 Data URIs
									}
									style={{
										width: 120,
										height: 120,
