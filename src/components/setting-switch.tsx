...
function SkeuoToggle({ value, enabled, onToggle }: { value: boolean; enabled: boolean; onToggle: () => void }) {
    ...
    const knobTranslate = anim.interpolate({ inputRange: [0, 1], outputRange: [2, 24] });
    ...
    return (
        <Pressable onPress={enabled ? onToggle : undefined} hitSlop={8}>
            ...
            >
                <Animated.View style={[toggle.knob, { transform: [{ translateX: knobTranslate }] }]}>
                    <LinearGradient
    ...
    return (
        ...
        >
            ...
            <View style={setitem.rightContainer} pointerEvents="none">
                <SkeuoToggle value={selectedValue} enabled={enabled} onToggle={handleToggle} />
            </View>
        </Pressable>
    );
}
